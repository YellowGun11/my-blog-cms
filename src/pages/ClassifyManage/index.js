...
import ClassifyManageEdit from './components/ClassifyManageEdit';
...
class ClassifyManage extends Component {
  ...
  getList = (searchParams, pagination) => {
    ...
    this.props.dispatch({ type: 'classify/getClassifyList', payload });
  };
  ...
  render() {
    ...
    const { dataList, pagination } = classify;
    return (
      <div>
        {/* 查询 */}
        <div style={{ textAlign: 'left', height: '60px' }}>
          <Form onFinish={this.searchData} onFinishFailed={this.onFinishFailed} layout="inline">
            <Form.Item colon={false} style={{ marginBottom: 0 }} name="name">
              <Input style={{ textAlign: 'left', width: '260px' }} placeholder="请输入分类名称" />
            </Form.Item>
            <Form.Item>
        ...
        <Modal
          ...
          title={whichPageShow === 'add' ? '新增分类' : '分类编辑'}
          destroyOnClose="true"
          width="520px"
          onCancel={(e) => {
            e.preventDefault();
